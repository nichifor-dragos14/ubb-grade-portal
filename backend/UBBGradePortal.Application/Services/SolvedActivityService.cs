...
public class SolvedActivityService : ISolvedActivityService
{
    ...
    public async Task<PaginatedProfessorSolvedActivityDto> GetAllByStatusForProfessorCourses(int pageNumber, int pageSize, SolvedActivityStatusFilter status, string? studentName, Guid? courseId, Guid loggedUserId, CancellationToken cancellationToken)
    {
        var statusFilter = status == SolvedActivityStatusFilter.All
            ? (SolvedActivityStatus?)null
            : (SolvedActivityStatus)status;

        var (Count, Activities) = await _solvedActivityRepository.GetAllByStatusForProfessorCourses(pageNumber, pageSize, statusFilter, studentName, courseId, loggedUserId, cancellationToken);
